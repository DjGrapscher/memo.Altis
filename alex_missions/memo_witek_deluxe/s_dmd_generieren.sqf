...
s_temp_dmd_haeuser = [_dm_area select 0, _dm_area select 1, s_dmd_min_gebaeudepositionen] call fnc_s_sys_gebaeudeauswahl; /* global, da auch in missionsteilnahme verwendet */
...
while {s_mission_params select 0} do {
  ...
  if (_anz_ki_erstellbar > 0) then {
    ...
    _positionen_innerhalb_haus = [_starthaus] call fnc_s_sys_positionen_innerhalb_haus;

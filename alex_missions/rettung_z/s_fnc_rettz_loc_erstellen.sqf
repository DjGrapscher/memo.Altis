...
for "_i" from 0 to 360 step 2 do {
  ...
  if ((!(surfaceiswater _pos_rel)) && {(_pos_rel distance (s_sys_area_hq select 0)) > (s_sys_area_hq select 1)}) then {
    _wasser_gefunden = [_pos_rel,position _zivilist] call fnc_s_sys_wasser_zwischen_a_und_b;
    if (!_wasser_gefunden) then {
      ...
      if (_pos_im_wald) then {
        ...
        _gebaeudeliste = [_pos_rel,50,_DEF_min_anz_geb_positionen_zombie_spawn] call fnc_s_sys_gebaeudeauswahl;
        if ((count _gebaeudeliste) > 0) then {
          _positionen_innerhalb_haus = [selectrandom _gebaeudeliste] call fnc_s_sys_positionen_innerhalb_haus;
